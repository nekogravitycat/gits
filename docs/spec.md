...
**條目順序**：`repos` 依 `name` 的位元組序排列。`gits add` / `gits adopt` **依序插入**，不附加於尾端——這讓兩台機器各自新增 repo 時落在檔案的不同位置，git 多半能自動 merge，而附加於尾端必然造成同一位置的衝突（見 §10.1）。手動編輯後可用 `gits fmt`（§7.13）把順序恢復。
