...
**條目順序**：`repos` 依 `name` 排列，規則與一般檔案總管相同——不分大小寫，且名稱中的數字段落依數值比較（例如 `repo2` 排在 `repo10` 之前），而非逐位元組比較。`gits add` / `gits adopt` **依序插入**，不附加於尾端——這讓兩台機器各自新增 repo 時落在檔案的不同位置，git 多半能自動 merge，而附加於尾端必然造成同一位置的衝突（見 §10.1）。手動編輯後可用 `gits fmt`（§7.13）把順序恢復。

**標準排版**：`gits` 寫出的 manifest 一律採用同一種排版，`gits fmt` 也把手改過的檔案還原成它。這不只是好看：排版固定，`gits.yaml` 的 diff 才只會出現真正的內容變動。

- 頂層鍵依序為 `version`、`defaults`、`repos`；`defaults` 內依序為 `remote`、`branch`。
- 每個 `repos` 條目的欄位依序為 `name`、`path`、`url`、`branch`、`remote`、`groups`、`no-write`、`description`——先是身分（叫什麼、在哪、從哪來），再來是 git 參數，最後是給人看的說明。此版本不認得的鍵**保留**並排在已知鍵之後。
- 條目之間空一行。十八個條目擠成一整塊時，肉眼很難看出一個條目在哪裡結束。
- `groups` 寫成單行的 `[game, platform]`；但若其中帶有註解則維持多行，因為單行形式沒有地方放註解。
- 註解一律保留，並跟著它所描述的條目一起移動。
...
**排版**：與 `gits.yaml` 同一套規則（§5.2），由 `gits fmt` 一併處理——`overrides` 依 `name` 排序、條目之間空一行、欄位依序為 `name`、`path`、`disabled`、`no-write`。這個檔案不進版控，所以排序不是為了 merge，而是單純為了在二十筆裡找得到某一筆；兩個並排的檔案不該讓讀者學兩套慣例。
...
v1 提供十三個指令，但**日常只需要記兩個動詞**：

| 分類 | 指令 | 用途 |
| --- | --- | --- |
| **日常** | `up`、`status` | 「把我拉到最新」與「現在怎麼樣」。九成的使用只會用到這兩個。 |
| 寫入 | `commit`、`push` | 收工前的提交與推送。 |
| 清單維護 | `init`、`adopt`、`add`、`clone`、`fmt` | 建立清單、登記新 repo、補齊缺少的 repo、把清單排版還原。 |
...
把 manifest 還原成標準排版：條目依 `name` 排序、條目之間空一行、欄位依固定順序、註解保留。

處理 `gits.yaml`（§5.2），以及這台機器有 `gits.local.yaml` 時一併處理它（§5.5）。本地檔不存在是常態，不是錯誤。

```
gits fmt
gits fmt --dry-run --exit-code    # 只檢查不寫入；需要排版時退出碼 3
```

- **不是純粹的美觀問題。** 條目依 `name` 排序是 §10.1 的合併策略本身：兩台機器各自新增的 repo 因此落在檔案的不同位置，git 自動 merge 就過了。手動編輯很自然地會把新條目加在尾端，`fmt` 是把它放回去的方式。
- 排版固定之後，`gits.yaml` 的 diff 只會出現真正的內容變動——沒有引號風格、縮排、欄位順序的雜訊。
- **註解跟著條目走。** 條目上方的註解是寫來說明那個條目的，排序移動條目時一併移動。
- **不認得的鍵保留**，排在已知鍵之後。會刪掉自己看不懂的東西的 formatter，沒有人敢放進 hook。
- 先載入並驗證兩個檔案（§5.6）；驗證失敗即以 `E_MANIFEST` 中止，兩個檔案都不動。看不懂的檔案不該重寫。
- 空的 `gits.local.yaml`（合法，代表「這台機器沒有覆寫」）原樣保留，不重塑。
- **冪等**：檔案已是標準排版時不寫入（連 mtime 都不動），輸出「already formatted」，退出碼 `0`。因此可以無條件放進 pre-commit hook。
- `--dry-run` 只回報不寫入；搭配 `--exit-code`，「需要排版」會得到退出碼 `3`（§6.10），這是 CI 檢查用的形式。
- `--json` 頂層輸出 `changed`（任一檔案需要改動），另以 `files[]` 逐檔輸出 `file`／`entries`／`changed`／`reordered`。`changed` 是呼叫者該分支的欄位。
- `gits init` / `add` / `adopt` 寫出的檔案本來就是標準排版，因此對它們剛寫過的 manifest 跑 `fmt` 必定是 no-op。
